...
function SpeciesMenu ({speciesArray, passSpeciesArrayToApp, passDeletedItemIDToApp, strictMode, readOnlyMode, isMarkingAnnotationArea, setIsMarkingAnnotationArea, silent }) {
    ...
    const handleClickClustername = (selectedID, selectedClustername) => {
        const modifiedSpeciesArray = speciesArray.map(speciesObject => {
            if (speciesObject.id === selectedID) {
                ...
                const fallbackIndividual = speciesObject.name === ANNOTATED_AREA ? ANNOTATED_AREA_INDIVIDUAL_TRAIN_WHISPERSEG :  speciesObject.individuals[0].name
                const updatedIndividuals = checkIfEveryObjectIsInactive(speciesObject.individuals)
    ...
    useEffect(() => {
        if (!isMarkingAnnotationArea){
            return
        }
        const annotatedAreaSpecies = speciesArray.find( species => species.name === ANNOTATED_AREA );
        handleClickIndividual(annotatedAreaSpecies.id, annotatedAreaSpecies.individuals[0]);

    }, [isMarkingAnnotationArea]);

    useEffect( ()=>{
        if (!speciesArray){
            setIsMarkingAnnotationArea(false);
        }else{
            const annotatedAreaSpecies = speciesArray.find( species => species.name === ANNOTATED_AREA );
            if (!annotatedAreaSpecies.individuals[0].isActive){
                setIsMarkingAnnotationArea(false);
            }
        }
    }, [ speciesArray ] )
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className='individual-btn-container'>
                                    <div style={{color:"#6d7479"}}>Individuals:</div>
                                    {
                                    ...
                                    {
                                        species.showIndividualInputWindow && !silent &&
                                            <InputWindow
                                ...
                                <div className='clustername-btn-container'>
                                    <div style={{color:"#6d7479"}}>Vocalizations:</div>
                                    {
                                    ...
                                    {
                                        species.showClusternameInputWindow && !silent &&
                                            <InputWindow
