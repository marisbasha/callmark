...
import {createSpeciesFromImportedLabels, ANNOTATED_AREA, ANNOTATED_AREA_INDIVIDUAL_TRAIN_WHISPERSEG} from "./species.js";
...
function ModelsWindow (
        {
            ...
            assignCurrentConfigToLabelsWithoutIt,
    ...
    )
{
    ...
    const [ignoreCluster, setIgnoreCluster] = useState(false)
    ...
    const filterAndConvertLabelsForWhisper = () => {
        ...
        return newLabelsArray.map( label => {
                const exportTime = binsToExportTime(label.onsetBin, label.offsetBin, label.configSnapshot.nfft, label.configSnapshot.hopLength, label.configSnapshot.samplingRate)
                return {
    ...
    const filterAndConvertAnnotatedAreasForWhisper = () => {
        ...
        return labels.reduce( (acc, label) => {
            if ( (label.species === ANNOTATED_AREA) && ( label.individual === ANNOTATED_AREA_INDIVIDUAL_TRAIN_WHISPERSEG )   ) {
                acc.push({
                    onset: label.onsetTime,
                    offset: label.offsetTime
                })
                return acc
            }
            return acc
                    ...
                    <div className='models-container'>
                        ...
                        <form className='models-form' onSubmit={handleClickSubmitTrainingRequestBtn}>
                            <div className='models-form-input-fields'>
                                ...
                                <label>
                                    <div>Predict Onset/Offset Only:</div>
                                    <Checkbox size="small"
                                              checked={ignoreCluster}
                                              onChange={(event)=>{ setIgnoreCluster( event.target.checked ) }}
